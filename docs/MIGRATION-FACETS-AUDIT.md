...
| **Measures / Metrics** | 60+ keys via Protobuf `Measure` (type-aware) | **NO measures migrate at all.** `ReportData.Measures` is hardcoded empty (`tasks_scanhistory.go:441`). `BuildMeasures` is **dead code** (test-only caller). Extraction requests only ~12 keys (feeding a local markdown report). "60+ keys" is fabricated. |
...
| **Source Code** | content, language, line count all migrate | Mechanism misattributed: language + line count are in **`component-{ref}.pb`**, not `source-{ref}.txt`. Gated behind `--include-scan-history` in the `migrate` command |

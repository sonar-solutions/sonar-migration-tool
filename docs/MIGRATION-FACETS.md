...
| **Analysis Data** | Source Code | raw file content; language; line count | content rides in `source-{ref}.txt`; **language + line count ride in `component-{ref}.pb`** (not the source file). In the `migrate` command this is gated behind `--include-scan-history` (default off); always on in `transfer` | `source-{ref}.txt` + `component-{ref}.pb` in report ZIP | 004 |
